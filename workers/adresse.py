...
def load_adresse(engine: create_engine, df_real_estate: pd.DataFrame, df_commune: pd.DataFrame) -> pd.DataFrame:
  print("---- T/L Adresse ----")
  try:
    # Transform
    ...
    # Rename
    df_adresse_clean = df_adresse_distinct.rename(columns={
        "No voie": "numero_voie",
    ...
    df_adresse_clean['code_insee'] = df_adresse_clean['id_departement'] + \
        df_adresse_clean['code_commune']

    df_adresse_merged = pd.merge(
        df_adresse_clean, df_commune, on="code_insee", how="inner")

    df_adresse_to_load = df_adresse_merged[["id_commune",
                                           "numero_voie", "nom_voie", "id_voie"]]
    ...
    df_adresse = pd.read_sql(select(models.Adresse),
                             engine)
