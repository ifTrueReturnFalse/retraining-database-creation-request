...
def load_region(engine: create_engine, df_geographic_ref: pd.DataFrame):
    ...
    try:
        ...
        # Rename
        df_regions_clean = df_regions_distinct.rename(
            columns={"reg_code": "id_region", "reg_nom": "nom"}
        )
        print(f"{len(df_regions_clean)} regions founded.")

        # Check
        df_actual = pd.read_sql(select(models.Region.id_region), engine)

        if not df_actual.empty:
            existing_ids = df_actual["id_region"].tolist()
            df_to_load = df_regions_clean[
                ~df_regions_clean["id_region"].isin(existing_ids)
            ]
        else:
            df_to_load = df_regions_clean
