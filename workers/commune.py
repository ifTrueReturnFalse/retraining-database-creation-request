...
def load_commune(
    ...
    df_geographic_ref: pd.DataFrame,
    df_commune_info: pd.DataFrame,
) -> pd.DataFrame:
    print("---- T/L Commune ----")
    try:
        # Transform
        ...
        # Rename
        df_commune_clean = df_commune_distinct.rename(
            columns={
        ...
        df_commune_info_clean = df_distinct_commune_info.rename(
            columns={
        ...
        df_commune_info_clean["code_insee"] = (
            df_commune_info_clean["id_departement"]
            + df_commune_info_clean["code_commune"]
        )
        df_commune_info_clean = df_commune_info_clean.drop(
            columns=["id_departement", "code_commune"]
        )

        df_commune_merged = pd.merge(
            df_commune_clean, df_commune_info_clean, on=["code_insee"], how="inner"
        )

        df_to_load = df_commune_merged[
            ["id_departement", "nom", "code_commune", "code_insee", "population"]
        ]
