...
def load_departement(engine: create_engine, df_geographic_ref: pd.DataFrame):
  ...
  try:
    ...
    # Rename
    df_departement_clean = df_departement_distinct.rename(columns={
    ...
    df_actual = pd.read_sql(
        select(models.Departement.id_departement),
        engine
    )

    if not df_actual.empty:
      existing_ids = df_actual['id_departement'].tolist()
      df_to_load = df_departement_clean[~df_departement_clean["id_departement"].isin(
          existing_ids)]
    else:
